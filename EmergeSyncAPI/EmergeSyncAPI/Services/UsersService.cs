...
namespace EmergeSyncAPI.Services;

public class UsersService
{
    private readonly IMongoCollection<User> _usersCollection;

    public UsersService(IOptions<EmergeSyncDBSettings> emergeSyncDBSettings)
    {
        var mongoClient = new MongoClient(emergeSyncDBSettings.Value.ConnectionString);
        var mongoDatabase = mongoClient.GetDatabase(emergeSyncDBSettings.Value.DatabaseName);
        _usersCollection = mongoDatabase.GetCollection<User>(emergeSyncDBSettings.Value.UsersCollectionName);
    }
